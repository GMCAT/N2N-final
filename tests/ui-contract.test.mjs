...
test("pairing UI creates or joins an eight-digit live room", async () => {
  ...
  assert.match(panel, /v1\.5\.1/u);
  assert.match(panel, /live\.channelOpen \? 30_000 : 2_000/u);
  assert.match(panel, /30_000/u);
  assert.match(panel, /connectionHealth/u);
  ...
  assert.match(panel, /filePickerOpenRef/u);
  assert.match(panel, /onCancel/u);
  ...
  assert.match(panel, /verificationExpiredRoomId/u);
  assert.match(panel, /verificationExpiresAt/u);
...
test("v1.1 streams large files with receiver consent and backpressure", async () => {
  ...
  assert.match(live, /setPeerLeftRoomId\(session\.id\)/u);
  assert.doesNotMatch(live, /if \(active && opened\) setPeerLeftRoomId/u);
  ...
  assert.match(live, /restartIce/u);
  ...
  assert.match(live, /kind: "ping"/u);
  assert.match(live, /kind: "pong"/u);
  assert.match(live, /15_000/u);
  assert.match(live, /60_000/u);
  assert.match(live, /signalPollBackoff/u);
  assert.match(live, /hasOpenedChannel/u);
  assert.match(live, /nextDelay = 850/u);
  ...
  assert.match(live, /nextDelay = 60_000/u);
  assert.match(rooms, /now - 45_000/u);

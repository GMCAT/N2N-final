...
type QueueTicket = { id: string; token: string; position: number; expiresAt: number };
...
export function PairingApp() {
    ...
    async function pollQueue() {
      try {
        const result = await responseJson<
          | { queued: true; queueId: string; position: number; expiresAt: number }
          | { queued: false; id: string; code: string; senderToken: string; expiresAt: number }
        >(await fetch(`/api/rooms/queue/${queueId}`, { method: "POST", headers: { Authorization: `Bearer ${queueToken}` } }));
        if (!active) return;
  ...
  const connected = roomStatus === "connected" && (live.channelOpen || peerOnline);
      ...
      {!session ? (
        ...
        <section className="room-layout">
          ...
          <section className="conversation" aria-label="พื้นที่รับส่งข้อมูล">
            ...
            {live.connectionHealth === "unstable" && <div className="verification-banner" role="status"><strong>การเชื่อมต่อไม่เสถียร</strong><span>ระบบกำลังตรวจสอบอีกฝ่าย หากไม่ตอบภายใน 15 วินาทีจะเชื่อมต่อใหม่อัตโนมัติ</span></div>}
            {!live.ready && (

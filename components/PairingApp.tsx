      ...
      {!session ? (
        ...
        <section className="room-layout">
          ...
          <section className="conversation" aria-label="พื้นที่รับส่งข้อมูล">
            ...
            {live.transferLabel && <div className="live-progress"><span>{live.transferLabel}</span><strong>{Math.round(live.progress * 100)}%</strong><progress max="1" value={live.progress} />{live.transferStats && <div className="transfer-stats"><span><small>ไฟล์</small><b>{live.transferStats.fileName}</b></span><span><small>ส่งแล้ว / ทั้งหมด</small><b>{formatBytes(live.transferStats.transferredBytes)} / {formatBytes(live.transferStats.totalBytes)}</b></span><span><small>ความเร็ว</small><b>{live.transferStats.mbps > 0 ? `${live.transferStats.mbps.toFixed(2)} Mbps` : "— Mbps"}</b></span><span><small>คาดว่าจะเสร็จ</small><b>{formatEta(live.transferStats.etaSeconds)}</b></span></div>}{session.role === "sender" && live.progress > 0 && <div className="transfer-actions"><button onClick={live.transferPaused ? live.resumeTransfer : live.pauseTransfer}>{live.transferPaused ? "ส่งต่อ" : "หยุดชั่วคราว"}</button><button onClick={live.cancelTransfer}>ยกเลิก</button></div>}</div>}
